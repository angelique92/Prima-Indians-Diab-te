...
def RDN(X_train, X_test, y_train, y_test):
	model=Sequential()
	model.add(Embedding(1000,264))
	model.add(LSTM(128,activation='sigmoid',return_sequences=True,dropout=0.2, recurrent_dropout=0.2))
	model.add(LSTM(64,activation="sigmoid",return_sequences=True,dropout=0.2, recurrent_dropout=0.2))
	...
	callback_list=[
		...
		callbacks.ReduceLROnPlateau(factor=0.7,monitor="val_accuracy",patience=3)
	]

	history=model.fit(X_train,y_train,validation_data=(X_test, y_test),epochs=100,verbose=1,callbacks=callback_list)
...
#________________IMPROVE_DATA_______________
